...
axios.post(`${HOST}/api/get-preferences-dashboard`,{
    ...
    useEffect(() => {
        getToken()
    navigation.getParent()?.setOptions({ tabBarStyle: { display: 'default'} });
 ...
 <e.PreferencesPopupDescriptionView><e.PreferencesPopupDescription>You are searching for: </e.PreferencesPopupDescription>
        ...
        <e.AccountsContent>
            ...
            business.map((usr,KEY)=>{
                return(
                    <e.AccountCardContent key={KEY} onPress={()=>{
                ...
                <e.AccountCard>
                    ...
                    <e.AccountCardName>{usr.fname}</e.AccountCardName>
                    <e.LocationView>
                    <e.LocationIc name="location-on" size={19} color="#000"/>
                    <e.AccountCardLocation>{usr.location}  {usr.ID} {usr.type}</e.AccountCardLocation>
                    </e.LocationView>
                </e.AccountCard>
                </e.AccountCardContent>
                )
            }) : users.map((usr,KEY)=>{
                return(
                 <e.AccountCardContent key={KEY} onPress={()=>{
