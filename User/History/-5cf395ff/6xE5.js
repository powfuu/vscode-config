...
export default function App() {
    ...
    const opacityModal = useState(new Animated.Value(0))[0]
 ...
 const fadeOut = (prop) =>{
  ...
  return (
      <e.MainView>
          {modalStatus === true ? <Animated.View style={[{opacity:opacityModal}]}><e.Modal>
               <e.AddTouchable onPress={()=>{
                   fadeOut(opacityModal)
        setTimeout(() => {
              ...
              <e.ModalView>
                {options.map(value => {
                    return(
                        <e.Debug>prop: {value}</e.Debug>
                    )
                })}
              </e.ModalView>
          </e.Modal></Animated.View> : null}
          <e.LogoView>
