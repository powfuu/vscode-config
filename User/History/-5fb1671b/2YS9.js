...
export default function Signup({navigation}){
  ...
  /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/;
    const signup = () =>{
        setRegStatus(true)
if(businessName.length >= 2 && email.match(mailformat) && password.length >=7 && confirmPassword == password && businessLastName.length >= 2){ 
    ...
    if(lastName.length < 2){
        error = 'Last name must be biggerthan 2 characters!'
        setbusinessLastName('#FF7979')
    }else{
