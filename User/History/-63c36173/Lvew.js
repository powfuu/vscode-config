...
const Decision = ({navigation: { goBack }}) =>{
    ...
    return(
        <e.MainView>
            <e.AddTouchable onPress={()=>{
                goBack()
            }}><e.GoBack name='ios-arrow-back-outline' color="#fff" size={35}/></e.AddTouchable>
            <e.ResultView>
            <e.Crown name="crown-outline" size={65} color="gold" />
            <LinearGradient
            colors={['#00FFFF', '#17C8FF', '#329BFF', '#4C64FF', '#6536FF', '#8000FF']}
            start={{x: 0.0, y: 1.0}} end={{x: 1.0, y: 1.0}}
            style={{ height: boxHeight, width: 200, alignItems: 'center', justifyContent: 'center', width: 200, borderRadius: 100}}>
             <e.ResultMainView>
              <e.ResultText>
                  {result[0]}
              </e.ResultText>
