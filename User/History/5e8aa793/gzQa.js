...
<e.AddTouchable onPress={()=>{
    ...
    <e.MenuList name="menu-open" size={33} color="white" />
    </e.AddTouchable>
        ...
        <Animatable.View animation="zoomInDown" delay={700} duration={1500}>
    <LinearGradient
          ...
          {insertingStatus === true ? <Animated.View style={[{opacity}]}>
            ...
            <e.AddOptionInputForm>
                ...
                <e.AddTouchable onPress={()=>{
                    ...
                    setNewOption('')
                }}><e.AddOptionSvg name="md-paper-plane-sharp" size={27} color="white" /></e.AddTouchable>
            </e.AddOptionInputForm>
</e.AddOptionView>
          </Animated.View> : null}
          <e.CenteredView>
...
<LinearGradient
    ...
    style={{ height: 57, width: 200, alignItems: 'center', justifyContent: 'center', width: 200, borderRadius: 100}}>
                  <e.GenerateAnswer underlayColor='rgba(73,182,77,1,0.9)' onPress={()=>{
              if(options.length === 0){
                ...
                {options.map((value,KEY) => {
                    return(
                        <e.OptionModal key={KEY}>
                            <e.OptionRow>
                            <e.OptionOptionSvg name="clear-all" color={'#fff'} style={{marginTop:'-1%'}} size={25}/>
